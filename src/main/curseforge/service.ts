...
export class CurseForgeService{
  ...
  async files(apiKey:string,modId:number,gameVersion?:string,loader?:string){const p=new URLSearchParams({pageSize:"50",index:"0",sortField:"2",sortOrder:"desc"});if(gameVersion)p.set("gameVersion",gameVersion);if(loader)p.set("modLoaderType",loader);return this.get(apiKey,`/mods/${modId}/files?${p}`);}
